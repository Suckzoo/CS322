...
void NFAMachine::CreateStateEdge()
{
	...
	for (i = 1; i <= instance->n; i++)
	{
		for (L = 0; L < instance->language.length(); L++)
		{
			if (!instance->edge[i][instance->language[L]].empty())
			{
				...
				if (instance->accept.find(instance->edge[i][instance->language[L]][0]) 
					!= instance->accept.end()) {
					ConnectState(m_pLabelToState[wstring(v1)].get(), m_pLabelToState[wstring(v2)].get(),
						[this, L](wstring *str, wchar_t x){return x == instance->language[L]; }, FlushBuffer);
				}
				else {
					ConnectState(m_pLabelToState[wstring(v1)].get(), m_pLabelToState[wstring(v2)].get(),
						[this, L](wstring *str, wchar_t x){return x == instance->language[L]; }, PushToStack);
				}
			}
		}
	}
	...
	for (i = 1; i <= instance->n; i++) {
		wchar_t v1[50];
		swprintf(v1, L"q%d", i);
		ConnectState(m_pLabelToState[wstring(v1)].get(), m_pInitialState.get(),
			[this](wstring *str, wchar_t x) {return x == DEL; }, onDeleteKey);
	}
}
